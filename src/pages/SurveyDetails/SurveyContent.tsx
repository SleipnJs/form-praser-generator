...
const SurveyContent = (props: any) => {
  ...
  const buildStats = data => {
    ...
    return props.responses[data]?.map((item: any) => {
      ...
      return (
        <p>
          <div style={{ height: '10px', width: '10px', backgroundColor: item.color, display: 'inline-block', marginRight: '10px' }}></div>
          {percent}% - {item.label}
        </p>
      );
    });
                ...
                if (props.responses[component.key] != null) {
                  return (
                    <div className={'form__content'}>
                      ...
                      <RadialChart data={props.responses[component.key]} width={300} colorType={'literal'} height={300} />
                      <div>{buildStats(component.key)}</div>
                    </div>
                  );
                } else {
                  return '';
